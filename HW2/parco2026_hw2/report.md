...
为了把 16 进程加速比从 baseline 的 ~9× 推向理论上限 16×，按以下顺序做了**渐进式优化，每一步单独 commit**（见 git tag `v0-baseline` … `v_final`），每一步本地多次中位数测量后保留有效项：
...
git tag v_final        # = v6（最终提交版）
```

### 5.3 最终主结果（$N = 8192$ 与 $N = 16384$）

#### 串行 vs 16 进程并行（7×median）

| 实现             | $N$    | min (s)  | **median (s)** | mean (s) | max (s)  |
| :--------------- | :----: | :------: | :------------: | :------: | :------: |
| serial           |  8192  | 0.03511  |  **0.03634**   | 0.03618  | 0.03684  |
| parallel ($p=16$)|  8192  | 0.00137  |  **0.00210**   | 0.00202  | 0.00226  |
| serial           | 16384  | 0.14258  |  **0.14575**   | 0.14607  | 0.15049  |
| parallel ($p=16$)| 16384  | 0.00635  |  **0.00747**   | 0.00756  | 0.00913  |

#### 加速比 $S = T_\text{serial}/T_\text{parallel}$

| 实现 | $N$    | $T_\text{serial}$ (s) | $T_\text{parallel}$ (s) | **$S$**   | $E = S/p$ |
| :--: | :----: | :-------------------: | :---------------------: | :-------: | :-------: |
| parallel      |  8192  |        0.0363         |          0.0021         | **17.31×** |   1.082   |
| parallel      | 16384  |        0.1457         |          0.0075         | **19.53×** |   1.220   |
...
固定 $N$，让 $p$ 在 $\{1, 2, 4, 8, 16\}$ 中变化，加速比 $S(p) = T(1)/T(p)$、效率 $E(p) = S(p)/p$（7×中位数）：

#### parallel.cpp（提交版，stored-A int16）

| $p$ | $T(p)$ (s) N=8192 | $S(p)$ | $E(p)$ | $T(p)$ (s) N=16384 | $S(p)$ | $E(p)$ |
| :-: | :-: | :-: | :-: | :-: | :-: | :-: |
|  1  | 0.01474 | 1.00 | 1.00 | 0.06346 | 1.00 | 1.00 |
|  2  | 0.00743 | 1.98 | 0.99 | 0.03209 | 1.98 | 0.99 |
|  4  | 0.00385 | 3.83 | 0.96 | 0.01620 | 3.92 | 0.98 |
|  8  | 0.00210 | 7.01 | 0.88 | 0.00828 | 7.67 | 0.96 |
| 16  | 0.00210 | 7.02 | 0.44 | 0.00747 | 8.50 | 0.53 |
...
2. **$p = 8$ 处仍能保持 $E \ge 0.88$**（v0 baseline 此时已掉到 0.66），int16 缩窄让 DRAM 带宽预算多撑了一个倍频；$N = 16384$ 时甚至接近理想（$E = 0.96$），因为单个 MPI 进程的工作集（256 MB int16 A）远超 L3，每核仍能拿到一份独立的 DRAM 通道。
3. **$p = 16$ 时 $E \approx 0.5$**：这是 single-socket DRAM 控制器被多个消费者塞满后的硬上限——v6 优化也无法越过这堵墙。
4. **$N=8192$ 在 p=8 → p=16 几乎不再加速**（7.01× → 7.02×）：因为此时每进程的 A 块只有 1024 行 × 8192 = 16 MB，已经能进 L3，再增加进程已无 DRAM 带宽压力可分摊；瓶颈彻底转移到了进程间同步。$N=16384$ 还有 32 MB/进程的工作集，所以 p=16 还能多挤出一点 ($S$ 从 7.67 → 8.50)。

### 5.5 为什么 16 进程下还达不到理想 $E=1$？—— Roofline 分析

GEMV 的算术强度（arithmetic intensity）极低。以 v0 / v6 两档分别算：
...
- $p = 8$ 时 v6 仍能 $E = 0.96$（N=16384），v0 baseline 此时只有 0.81——int16 缩窄确实拓宽了带宽空间；
- $p = 16$ 时 $E \approx 0.5$，因为此时 16 个进程已把单 socket 的 DRAM 控制器堵满；继续加进程对吞吐没帮助。
- **N=8192 在 p=8 → p=16 几乎完全没加速**（$T$ 从 2.10ms 维持到 2.10ms）：每进程局部 A 已缩到 16 MB 进入 L3，DRAM 不再是瓶颈，而 MPI 同步与进程启动/调度开销在 ~2 ms 量级显著体现。
...
2. **当前瓶颈是 DRAM 带宽**：v6 已证明每进程读完自己的 A 是限制因素。2D 块划分让每进程的 A 块更小（$\frac{N}{\sqrt p} \times \frac{N}{\sqrt p}$ vs $\frac{N}{p} \times N$），但**总读字节数**不变（仍是 $N^2$），并不解决带宽墙。
...
```bash
make                                            # 用 mpiicc 构建 parallel + serial + serial_timed
...
- **三个算例全部通过**：算例 S 与串行**逐元素一致**；算例 M（$N = 1000$，不整除 16）`verify.py` 输出 `PASS`；算例 L（$N = 8192$）的 16 进程加速比中位数 **17.31×**；额外测的 $N = 16384$ 加速比 **19.53×**。
- 因为 `serial.cpp`（基线）保留题目原始的 `long long` 算法、`parallel.cpp` 已用 int16 缩窄，所以加速比超过 16× 是**算法 + 并行的复合收益**。从纯并行角度看（强缩放 $T(1)/T(p)$），$E$ 在 $p=4$ 处仍 $\ge 0.96$，$p=8$ 处可达 $0.96$（N=16384），$p=16$ 处 $E \approx 0.5$，符合带宽-bound GEMV 的 Roofline 预期。
